...
parser <- add_argument(parser, "--resolution", default=NULL,
                       help='Specify the resolution of the matrix in bp. E.g.: For 10kb resolution put --resolution 10000')
...
main <- function() {
  ...
  #catch error: no resolution
  if(is.na(args$resolution)){
    stop('Resolution of the matrix not supplied. Please specify the resolution after the --resolution flag')
  }
  #catch error: NA cutoff out of bounds
  if(args$cutoff_NA >= 1 | 
     args$cutoff_NA < 0 ){
  ...
  #determine matrix resolution from command line input and IS length scales
  matrix_resolution <- as.integer(args$resolution)
  ...
  genomic_intervals[, 'end_bin' := fifelse(end %/% matrix_resolution == end / matrix_resolution,
                                           (end %/% matrix_resolution * matrix_resolution),
                                            (end %/% matrix_resolution * matrix_resolution) + matrix_resolution)]
  
  
  
  #initialize a data frame to which mean contacts can be added
  loop_out <- data.table(chrom = character(), start=integer(), end=integer(), ID = character(), ks_pval_raw_melt=double(), ks_pval_raw_condense=double(), perc_na_or_missing_ref=integer(), perc_na_or_missing_inp=integer())
  ...
  for(element in genomic_intervals$ID){
    ...
    if( args$cutoff_NA  >=  perc_na_or_missing_ref &
      ...
      suppressWarnings(ks_pval_melt <- ks.test(IS_table_subset$IS_ref,
                             IS_table_subset$IS_inp,
                             alternative = 'less')$p.value)
      suppressWarnings(ks_pval_condense <- ks.test(IS_table_subset$IS_ref,
                                               IS_table_subset$IS_inp,
      ...
      loop_out <- rbind(loop_out, list(chrom = element_row$chrom,
                                     ...
                                     ID = element_row$ID,
                                     ks_pval_raw_melt = ks_pval_melt,
                                     ks_pval_raw_condense = ks_pval_condense,
  ...
  loop_out[, ks_pval_corrected_melt := p.adjust(ks_pval_raw_melt, method = "bonferroni")]
  loop_out[, ks_pval_corrected_condense := p.adjust(ks_pval_raw_condense, method = "bonferroni")]
  
  
  #calculate melting score from corrected pval
  loop_out[, melting_score:= sprintf(-log10(ks_pval_corrected_melt), fmt='%#.2f')]
  loop_out[, condensation_score:= sprintf(-log10(ks_pval_corrected_condense), fmt='%#.2f')]
  file_out <- loop_out[, .(chrom, start, end, ID, melting_score, condensation_score, perc_na_or_missing_ref, perc_na_or_missing_inp)]
