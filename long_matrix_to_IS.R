...
main <- function() {
  ...
  for (file in args$input) {
    ...
    #determine first and last bins of that chromosome (how long is the chromosome)
    matrix_minmax <- range(matrix_long$A_start)
    
    #drop the NAs from the table
    matrix_long <- matrix_long[!is.na(matrix_long$count)]
    
    #determine matrix resolution from command line input
    matrix_res <- as.integer(args$resolution)
    ...
    if(IS_size_max < IS_size_min){
      stop('--ISmax is smaller than --ISmin')
    }
    
    #determine which chromosome is read in from filename
    splitted_filename <- str_split(file, "_")
    chromosome_ID_withFilesuffix <- splitted_filename[[1]][str_detect(unlist(splitted_filename), '^chr')][1]
    chromosome_ID_withFilesuffix_splitted <- str_split(chromosome_ID_withFilesuffix, "\\.")
    chromosome_ID <- chromosome_ID_withFilesuffix_splitted[[1]][str_detect(unlist(chromosome_ID_withFilesuffix_splitted), '^chr')][1]
    ...
    for(IS_distance in IS_size_min_to_max){
      ...
      for(viewpoint in unique(matrix_long_no_diagonal_filtered$A_start)){
        ...
        if(length(matrix_sub$count) >= expected_value_count/2 & 
           viewpoint > matrix_minmax[1] + IS_distance & #do not add to list if viewpoint is too close to start or end of chromosome
           viewpoint < matrix_minmax[2] - IS_distance){
